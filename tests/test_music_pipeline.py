...
def _make_task_with_dups(item_path: str, duplicates: list) -> MagicMock:
    """Build a mock ImportTask whose find_duplicates returns *duplicates*."""
    task = MagicMock()
    task.item = _item(item_path)
    task.find_duplicates.return_value = duplicates
    return task


def test_duplicate_action_no_duplicates_does_nothing() -> None:
    plugin = _make_plugin()
    task = _make_task_with_dups("/root/Music/inbox/spotdl/jazz/track.m4a", [])

    plugin.duplicate_action(MagicMock(), task)

    task.set_choice.assert_not_called()


def test_duplicate_action_all_spotdl_does_not_skip() -> None:
    plugin = _make_plugin()
    task = _make_task_with_dups(
        "/root/Music/inbox/spotdl/jazz/track.m4a",
        [_dup(source="jazz", via="spotdl")],
    )
...
def test_duplicate_action_manual_dup_calls_set_choice_skip() -> None:
    plugin = _make_plugin()
    task = _make_task_with_dups(
        "/root/Music/inbox/spotdl/jazz/track.m4a",
        [_dup(via="")],
...
def test_duplicate_action_find_duplicates_exception_does_nothing() -> None:
    plugin = _make_plugin()
    task = MagicMock()
    task.item = _item("/root/Music/inbox/spotdl/jazz/track.m4a")
    task.find_duplicates.side_effect = Exception("db error")
